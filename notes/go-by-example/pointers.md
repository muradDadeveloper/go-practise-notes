...
The `&i` syntax gives the memory address of `i`, i.e. a pointer to `i`. Pointers can be printed too. `zeroval` doesn’t change the `i` in `main`, but `zeroptr` does because it has a reference to the memory address for that variable.
